...
3. You will need these sign-in details:
   - **Username:** `user`
   - **Password:** `password`
...
**Steps:**
1. On the sign-in screen, type `user` into the Username box.
2. Type `password` into the Password box.
...
## Test 3 — Looking at the board

**Steps:**
1. Sign in normally (Test 1).
2. Look at the main area of the screen.

**What you should see:** Five columns side by side, each with a name at the top (things like "Backlog," "Discovery," "In Progress," "Review," "Done" — the exact names may have been changed by someone else). Each column has one or more cards in it, and each card has a short title and some details text.

☐ Pass ☐ Fail — Notes: ______________________

---

## Test 4 — Renaming a column
...
## Test 7 — Moving a card by dragging
...
## Test 8 — Removing the test card

**Steps:**
1. Find the test card.
2. Click **Remove** (sometimes shown as a trash icon or "Delete").

**What you should see:** The card disappears immediately and does not come back after reloading the page. This step cleans up the test data from Tests 5–7.

☐ Pass ☐ Fail — Notes: ______________________

---

## Test 9 — Asking the AI Assistant a question
...
## Test 10 — Asking the AI Assistant to make a change
...
**What you should see:** The assistant replies confirming what it did, and the new card actually appears on the board in the column you asked for — without you needing to reload the page.

**Cleanup:** Delete the card the assistant created (see Test 8) once you've confirmed it worked.

☐ Pass ☐ Fail — Notes: ______________________

---

## Test 11 — Closing and reopening the assistant panel
...
## Test 12 — Staying signed in after a page reload

**Steps:**
1. While signed in, reload the page (F5).

**What you should see:** You stay signed in and see the board again — you are **not** sent back to the sign-in screen.
...
## Test 14 — Trying to create a card with no title
...
## Test 15 — Typing an extremely long card title
