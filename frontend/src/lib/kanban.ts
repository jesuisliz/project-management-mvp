...
export const getCardDestination = (
  ...
  const currentPosition = currentColumn?.cardIds.indexOf(activeId) ?? -1;
  ...
  const destinationPosition = destinationColumn.cardIds.indexOf(activeId);
  if (
    destinationColumn.id === currentColumnId &&
